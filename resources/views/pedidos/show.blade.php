...
@section('content')
    ...
    <br>
    {!! $stepper !!}
    @if(($pedido->status == 'Diagramação' and Auth::user()->can('editora')) or ($pedido->status == 'Impressão' and Auth::user()->can('grafica')) or ($pedido->status == 'Diagramação' or $pedido->status == 'Impressão' and Auth::user()->can('owner.pedido', $pedido)))
      <br><br>
